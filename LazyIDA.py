...
def hex_reverse_tostring(data_list):
    result = ''
    for hex_str in data_list:
        byte_strs = [hex_str[i:i + 2] for i in range(0, len(hex_str), 2)]
        byte_strs.reverse() #Little-Endian
        result += bytes.fromhex("".join(byte_strs)).decode('utf-8', errors='replace')
    return result

def hex_reverse_toWstring(data_list):
    wide_char_array = []
    for hex_str in data_list:
        byte_strs = [hex_str[i:i + 2] for i in range(0, len(hex_str), 2)]
        byte_strs.reverse() #Little-Endian
        current_length = len(byte_strs)
        padding_size = (4 - current_length % 4) % 4
        byte_strs.extend(['00'] * padding_size)
        hex_data = "".join(byte_strs)
        hex_groups = [hex_data[i:i + 4] for i in range(0, len(hex_data), 4)]
        for hex_group in hex_groups:
            wide_char = unpack('<H', bytes.fromhex(hex_group))[0]
            wide_char_array.append(wide_char)
    return bytes(wide_char_array)
        ...
        if self.action in ACTION_CONVERT:
            ...
            if data:
                ...
                if self.action == ACTION_CONVERT[0]:
                    ...
                    data = []
                    while(pointer < end):
                        if(idc.get_operand_type(pointer,1) == 5):
                            operand2st = idc.print_operand(pointer,1)
                            data.append(hex_cleaner(operand2st))
                        pointer += idc.get_item_size(pointer)
                    print(hex_reverse_tostring(data))
                    ...
                    while(pointer < end):
                        if(idc.get_operand_type(pointer,1) == 5):
                            ...
                            data.append(hex_cleaner(operand2st))
                        pointer += idc.get_item_size(pointer)
                    print(hex_reverse_toWstring(data))
...
class LazyIDA_t(idaapi.plugin_t):
    ...
    def init(self):
        ...
        menu_actions = (
            ...
            idaapi.action_desc_t(ACTION_CONVERT[10], "Convert second operands to string",
                                 menu_action_handler_t(ACTION_CONVERT[10]), None, None, 80),
            idaapi.action_desc_t(ACTION_CONVERT[11], "Convert second operands to wstring",
                                 menu_action_handler_t(ACTION_CONVERT[11]), None, None, 80),
        ...
        if idaapi.init_hexrays_plugin():
            ...
            addon.name = "LazyIDA"
            addon.producer = "Lays"
            addon.url = "https://ithub.com/L4ys/LazyIDA"
